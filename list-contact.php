...
<?php echo "Welcome back, ".$_SESSION['user_contact']['name']; ?>, 
<a href="logout.php"><b>logout</b></a>
<div class="row" >
    <div class="col-md-3">
        <div class="list-group">
            <a href="#" class="list-group-item active">Menu</a>
            <a href="#" class="list-group-item">List Contacts </a>
...
<?php
    $user_id = $_SESSION['user_contact']['id'];
   $query = mysqli_query($connect, "SELECT * FROM all_contacts WHERE user_id='{$user_id}'") or die(mysqli_error());
    if (mysqli_num_rows($query) > 0 ) {
        echo "<table class='table'>
        <tr><th>S/N</th><th>Name</th><th>Email </th><th>Phone Number</th><th>Address</th>
        <th>Favorite</th>
        </tr>";
        $i = 0;
        while($fetch = mysqli_fetch_array($query)) {
            echo "<tr><td>".($i+1)."</td><td>".$fetch['name']. "</td><td>".$fetch['email']. "</td><td>".$fetch['phone']. "</td><td>".$fetch['address']. "</td><td>".$fetch['favourites']."</td></tr>";
            $i++;
        }
        echo "</table>";
    } else {
